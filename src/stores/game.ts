...
export const useGame = create<GameState>((set, get) => ({
  ...
  whyCannotDeploy(hex) {
    ...
    if (get().currentPlayer === 1 && hex.r < bottomLimit)
      return 'Außerhalb deiner Aufstellungszone.';
    return null;
  },
  ...
  placeObjectives(): ObjectiveMarker[] {
    ...
    while (objectives.length < numObjectives && attempts < maxAttempts) {
      ...
      const tooClose = objectives.some((obj) => axialDistance(candidate, obj.position) < 3);
  ...
  endActivation() {
    ...
    const cur = s.currentPlayer,
      other = cur === 0 ? 1 : 0;
    const hasOther = s.units.some(
      (u) => u.owner === other && (u as any).activated !== true && u.position,
    );
    const hasCur = s.units.some(
      (u) => u.owner === cur && (u as any).activated !== true && u.position,
    );
    if (hasOther) {
      set({ currentPlayer: other });
    } else if (hasCur) {
      set({ currentPlayer: cur });
    } else {
  ...
  getValidTargets(unitId) {
    ...
    for (const target of state.units) {
      ...
      if (dist === 1) {
        const hasMeleeWeapons = u.weapons.some(
          (w) => (w.range || 0) === 0 && !u.usedWeapons?.includes(w.name),
        );
        if (hasMeleeWeapons) {
  ...
  canUnitShoot(unitId) {
    ...
    const hasRangedWeapons = u.weapons.some(
      (w) => (w.range || 0) > 0 && !u.usedWeapons?.includes(w.name),
    );
